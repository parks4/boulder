...
- **Format version** — Boulder reads and writes **STONE 2.x** (`STONE_FORMAT_VERSION`, `"MAJOR.MINOR"`, versioned independently of the Boulder package version). `migrate_stone_config(raw)` runs first in `normalize_config` and again on the authored ruamel tree in `merge_config_into_yaml`: it drops `LEGACY_METADATA_KEYS`, warns on a newer/foreign version and stamps `metadata.stone_version`. Notices go to the `boulder.config` logger and to the `warnings` list of `/api/configs/parse`, `/upload` and `/sync` (shown in the YAML pane banner). STONE 1.x (top-level `nodes:`, `connections:`, `groups:`) is rejected with an actionable error. See [STONE_SPECIFICATIONS.md](STONE_SPECIFICATIONS.md) §1.
- **Compatibility policy** — within a STONE MAJOR, this Boulder's reader at `M.y` loads every file written at `M.x` (`x <= y`). Removing or renaming a STONE key needs a load-time migration (an entry in `LEGACY_METADATA_KEYS`, or a step in `migrate_stone_config`) plus a test in `tests/test_legacy_metadata_keys.py`; never a hard error. Any metadata key Boulder injects itself must be listed in `result_cache._RUN_LABEL_METADATA_KEYS` or the cache fingerprint splits.
...
- **Pipeline** (order matters): STONE migration (legacy keys, version stamp) → dialect detection → STONE v2 normalisation → `expand_composite_kinds` (plugin unfolders) → connection ordering (e.g. PressureController after its `master` MFC) → default group synthesis → return.
...
- **Release** — [.github/workflows/release.yml](.github/workflows/release.yml): on `release: published`, `python -m build`, attach wheel + sdist (`fetch-depth: 0` for setuptools_scm). Tags are plain `MAJOR.MINOR.PATCH`; the STONE format version (`config.STONE_FORMAT_VERSION`) is versioned independently of the Boulder package version, see AGENTS.md.
