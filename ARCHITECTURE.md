...
- **Release** — [.github/workflows/release.yml](.github/workflows/release.yml): on `release: published`, `python -m build`, attach wheel + sdist (`fetch-depth: 0` for setuptools_scm). Tags are plain `MAJOR.MINOR.PATCH`; Boulder MAJOR == STONE MAJOR (`config.STONE_FORMAT_VERSION`), see AGENTS.md.
