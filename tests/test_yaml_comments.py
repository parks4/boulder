...
"""
    ...
    def sample_internal_config(self):
        """Sample internal configuration format."""
        return {
            "metadata": {
                "name": "Test Configuration",
                "description": "Sample configuration with comments and units",
    ...
    def test_round_trip_conversion(self, sample_yaml_with_comments, sample_internal_config):
...
class TestYAMLFileOperations:
    ...
    def test_file_round_trip(self, tmp_path, sample_yaml_with_comments):
        """Test saving and loading YAML files with comment preservation."""
        from boulder.config import save_config_to_file_with_comments, load_config_file_with_comments
        
        # Create a temporary file
        temp_file = tmp_path / "test_config.yaml"
        
        # Write the original YAML
        temp_file.write_text(sample_yaml_with_comments)
        
        # Load with comments
        loaded_data = load_config_file_with_comments(str(temp_file))
        
        # Verify the data was loaded correctly
        assert loaded_data["metadata"]["name"] == "Test Configuration"
