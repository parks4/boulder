...
def get_layout(
    ...
    This function generates the complete HTML layout for the Boulder application,
    including all UI components, modals, and interactive elements. When debug
    mode is enabled, it also includes the console forwarding script to capture
    browser console messages.

    Args:
        initial_config: The initial reactor network configuration
        cyto_stylesheet: Cytoscape graph styling configuration
        original_yaml: Original YAML configuration with comments
        config_filename: Name of the loaded configuration file
        debug: If True, includes console forwarding script for debugging

    Returns
    -------
        html.Div: The complete application layout as a Dash HTML component

    Debug Mode:
        When debug=True, the layout includes a script tag that loads
        console_forwarding.js from the assets folder to enable browser
        console message forwarding to the server.
    """
    # Create the main layout children
    layout_children = [
        ...
        dbc.Modal(
            [
                dbc.ModalHeader("Current Configuration - YAML with 🪨 STONE Standard"),
                dbc.ModalBody(
                    [
    ...
    # Conditionally include console forwarding script
    if debug:
        layout_children.append(html.Script(src="/assets/console_forwarding.js"))
